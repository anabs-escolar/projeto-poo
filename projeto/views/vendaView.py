...
class VendaView:
    def vendas_listar(is_carrinho=False, carrinho_only=False, cliente_id=None):
        vendas = VendaDAO.listar(
            is_carrinho=is_carrinho, carrinho_only=carrinho_only, cliente_id=cliente_id
        )
        itens = VendaItemDAO.listar()
        produtos = ProdutoDAO.listar()

        return {"vendas": vendas, "itens": itens, "produtos": produtos}
    ...
    def verifica_estoque(produto_id):
        produto = ProdutoDAO.busca_obj(produto_id)
        if produto is None:
    ...
    def carrinho_visualizar(cliente_id):
        carrinho = None
        for v in VendaView.vendas_listar(is_carrinho=True, cliente_id=cliente_id)[
            "vendas"
        ]:
        ...
        for i in itens:
            for p in produtos:
                if i.getProduto() == p.getId():
                    p.setEstoque(p.getEstoque() - i.getQtd())
